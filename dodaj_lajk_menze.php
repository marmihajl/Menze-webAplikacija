...
if(!empty($_POST)&&$uredi=="t"){
    ...
    $upit = "update lajkovi_menza set menza=$meni, korisnik=$korisnik, datum='$pocetak' where menza=$menza2 and korisnik=$korisnik2 and datum = '$datum2';";
...
elseif(!empty($_POST)&&$uredi=="f"){
    ...
    $korisnik = $_POST["korisnik"];
    $pocetak = $_POST["pocetak"];
    $upit = "insert into lajkovi_menza values ($meni,$korisnik,'$pocetak');";
    $baza->ostali_upiti($upit);
    header("Location:postavke.php");
}
?>
    ...
    <body>
        ...
        <div class="sadrzaj">
            ...
            <form name="registracija" id="registracija" method="post" action="<?php echo $_SERVER["REQUEST_URI"]; ?>">
                <table>
                                <tr><td>Menza:</td><td><select id="meni" name="meni">
                                            <option></option>
                                            <?php $upit = "select * from menze;";
                                            $rez = $baza->select_upit($upit);
                                            while($r = mysqli_fetch_array($rez)){?>
                                            <option value="<?php echo $r[0];?>"<?php if($uredi=="t"&&$r[0]==$menza2){echo 'selected="selected"';} ?>><?php echo $r[1];?></option>
                                            <?php } ?>
                            </select></td></tr>
